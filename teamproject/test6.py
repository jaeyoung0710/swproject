...
class RoadDrawer:
    ...
    def add_road_labels(self, labels):
        ...
        for i, (x, y) in enumerate(label_positions):
            label = QLabel(labels[i], self.parent)
            label.setGeometry(int(x), int(y), 100, 30)
            label.setAlignment(Qt.AlignCenter)
            label.setStyleSheet("font-weight: bold; font-size: 16px; background: rgba(255,255,255,180); border-radius: 8px;")
            label.show()
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Custom Signal Times per Direction")
        ...
        #self.road_drawer.add_road_labels(["North", "East", "South", "West"])

        self.vehicles = []
        self.add_detected_vehicles({"north": 20, "south": 15, "east": 14, "west": 17})
